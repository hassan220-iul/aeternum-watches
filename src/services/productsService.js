...
// Supabase's `products` table uses snake_case column names, but the rest
// of the app (mock data, local-mode store, every component) was built
// around the camelCase shape below. This maps a raw Supabase row onto
// that shape so fields like rating/review count/case material actually
// show up once a real Supabase project is connected.
...
export async function createProduct(payload) {
  if (!isSupabaseConfigured) {
    const row = { id: genId('prod'), created_at: nowIso(), is_active: true, currency: 'USD', ...payload };
