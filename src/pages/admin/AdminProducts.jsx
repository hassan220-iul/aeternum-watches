...
const emptyForm = {
  name: '', slug: '', price: '', category: '', collection: '', stock: '', image_url: '', model_url: '',
  description: '', movement: '', caseMaterial: '', waterResistance: '', warranty: '',
};
...
export default function AdminProducts() {
  ...
  function openEdit(p) {
    ...
    setForm({
      ...
      description: p.description || '', movement: p.movement || '', caseMaterial: p.caseMaterial || '',
      waterResistance: p.waterResistance || '', warranty: p.warranty || '',
    });
    setModalOpen(true);
  }
  ...
  async function handleSave(e) {
    ...
    const payload = {
      ...
      caseMaterial: form.caseMaterial || null,
      waterResistance: form.waterResistance || null,
            ...
            <form onSubmit={handleSave}>
              ...
              <label>
                ...
                <textarea
                  ...
                  style={{ width: '100%', background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#f5f2ea', padding: '0.6rem 0.8rem', fontFamily: 'inherit' }}
                />
              </label>
              <label>Movement<input value={form.movement} onChange={(e) => setForm({ ...form, movement: e.target.value })} placeholder="e.g. Automatic, Cal. 1959" /></label>
              <label>Case Material<input value={form.caseMaterial} onChange={(e) => setForm({ ...form, caseMaterial: e.target.value })} placeholder="e.g. 18k Rose Gold, Titanium" /></label>
              <label>Water Resistance<input value={form.waterResistance} onChange={(e) => setForm({ ...form, waterResistance: e.target.value })} placeholder="e.g. 100m / 10 ATM" /></label>
              <label>Warranty<input value={form.warranty} onChange={(e) => setForm({ ...form, warranty: e.target.value })} placeholder="e.g. 5-year international warranty" /></label>
