...
// the Supabase platform at runtime — no need to set them yourself.
//
// Called by src/services/paymentService.js when the frontend wants to take
// payment for an order. Creates a Stripe-hosted Checkout Session and stores
// its id against the order so the webhook (stripe-webhook/index.js) can
// reconcile payment status once Stripe confirms it.
      ...
      line_items: [
        {
          price_data: {
            ...
            // Amount comes from the order row stored in the database, set
            // when the order was created — never from anything the browser
            // sends in this request, so it can't be tampered with client-side.
